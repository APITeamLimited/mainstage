...
export class PersistenceDoc {
  ...
  getUpdates() {
    ...
    return this.rp.redis
      ...
      .then(
        /** @type {function(Array<Buffer>)} */ (updates) => {
          logger(
            'Fetched ',
            logging.BOLD,
            logging.PURPLE,
 ...
 */
const createRedisInstance = (redisOpts, redisClusterOpts) =>
  redisClusterOpts
...
export class RedisPersistence extends Observable {
   ...
   */
  constructor({
    redisOpts = /** @type {any} */ (null),
    redisClusterOpts = /** @type {any} */ (null),
  } = {}) {
    super()
    this.redis = createRedisInstance(redisOpts, redisClusterOpts)
    this.sub = /** @type {Redis.Redis} */ (
      createRedisInstance(redisOpts, redisClusterOpts)
    )
    /**
  ...
  closeDoc(name) {
    ...
    const bindCount = this.bindCount.get(name)
    if (doc && bindCount <= 1) {
