...
import { mongoPersistence, redisPersistence } from '../persistence-provider'
...
/*
const openDoc = map.setIfUndefined(openDocs, docName, async () => {
    ...
    doc.on(
      'update',
      async (update) => await mongoPersistence.storeUpdate(docName, update)
    )
...
const getOpenDoc = async (scope: Scope): Promise<OpenDoc> => {
