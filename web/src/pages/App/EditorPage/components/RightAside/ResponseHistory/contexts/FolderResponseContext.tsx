...
type FolderResponseContextProps = {
  children?: React.ReactNode
  collectionYMap: YMap<any>
  includeAll?: boolean
}

export const FolderResponseContext = ({
  children,
  collectionYMap,
  includeAll,
}: FolderResponseContextProps) => {
  ...
  const historyYMap = getOrCreateSubYMap(Y, collectionYMap, 'folderResponses')
  ...
  const handleDeleteResponse = useCallback(
    (responseId: string) => {
      const folderResponse = historyYMap.get(responseId) as YMap<any>

      // If response is for a running request, cancel the request
      if (folderResponse.get('__subtype') === 'LoadingResponse') {
        cancelRunningTest?.(
          workspaceInfo.isTeam ? workspaceInfo.scope.variantTargetId : null,
          folderResponse.get('jobId'),
          folderResponse.get('executionAgent'),
          folderResponse.get('localJobId')
        )
      }

      clearFocusedResponse(focusedElementDict, folderResponse)
