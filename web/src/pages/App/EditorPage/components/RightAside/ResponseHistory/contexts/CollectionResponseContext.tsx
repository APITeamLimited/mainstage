...
type CollectionResponseContextProps = {
  children?: React.ReactNode
  collectionYMap: YMap<any>
}

export const CollectionResponseContext = ({
  children,
  collectionYMap,
}: CollectionResponseContextProps) => {
  ...
  const historyYMap = getOrCreateSubYMap(
    Y,
    collectionYMap,
    'collectionResponses'
  )
  ...
  const groupedResponses = useMemo(() => {
    const responses: YMap<any>[] = (
      ...
      const subtype = response.get('__subtype')

      return (
        subtype === 'FailureResult' ||
        subtype === 'LoadingResponse' ||
        subtype === 'SuccessMultipleResult'
      )
    })
  ...
  const handleDeleteResponse = useCallback(
    (responseId: string) => {
      const collectionResponse = historyYMap.get(responseId) as YMap<any>

      // If response is for a running request, cancel the request
      if (collectionResponse.get('__subtype') === 'LoadingResponse') {
        cancelRunningTest?.(
          workspaceInfo.isTeam ? workspaceInfo.scope.variantTargetId : null,
          collectionResponse.get('jobId'),
          collectionResponse.get('executionAgent'),
          collectionResponse.get('localJobId')
        )
      }

      clearFocusedResponse(focusedElementDict, collectionResponse)
